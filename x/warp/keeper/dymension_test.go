...
var _ = Describe("dymension collat.go", Ordered, func() {
...
var _ = Describe("dymension synth.go", Ordered, func() {
	var s *i.KeeperTestSuite
	var owner i.TestValidatorAddress
	var sender i.TestValidatorAddress
	...
	It("MsgRemoteTransfer && MsgRemoteReceiveSynthetic (Memo) (dummy hook) (valid)  (Synthetic)", func() {
		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ DYMENSION CHANGE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Need to do this at the start to register the app router modules
		h := &DummyHook{}
		dymHandler := keeper.NewDymensionHandler(&s.App().WarpKeeper)
		...
		tokenId, mailboxId, igpId, _ := createToken(s, &remoteRouter, owner.Address, sender.Address, types.HYP_TOKEN_TYPE_SYNTHETIC_MEMO)
