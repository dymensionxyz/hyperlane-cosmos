...
for dir in $proto_dirs; do
  for file in $(find "${dir}" -maxdepth 1 -name '*.proto'); do
    ...
    if grep -q "option go_package" "$file" && grep -H -o -c 'option go_package.*github.com/dymensionxyz/hyperlane-cosmos/api' "$file" | grep -q ':0$'; then
...
cp -r github.com/dymensionxyz/hyperlane-cosmos/* ./
rm -rf api && mkdir api
mv hyperlane/* ./api
